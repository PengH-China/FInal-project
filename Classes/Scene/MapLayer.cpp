...
{
	const int randomBoxNum = cocos2d::random(4, 6);
...
void MapLayer::updatePosition(float dt)
{
	...
	else if (globalHero->getScore()>=20)
	{//five minutes
		Size visibleSize = Director::getInstance()->getVisibleSize();
		RenderTexture* renderTexture = RenderTexture::create(visibleSize.width, visibleSize.height);
		renderTexture->begin();
...
void MapLayer::addAIHero()
{
	int AIHeroNum = 9;

	for (int i = 0; i <= AIHeroNum; i++) {
		log("num[%d]=%d", i, num[i]);
		if (i == 5) {
			continue;
		}
		...
		switch (num[i]) {
			case 0: {
				AIhero = AIHero::createAIHero(Vec2(0, 0), "Hero1");
				if (AIhero == nullptr)
				{
				...
				else
				{
					...
					const int posX = cocos2d::random(32, maxWidth - 32);
					const int posY = cocos2d::random(32, maxHeight - 32);
					AIhero->setPosition(posX, posY);
					addChild(AIhero, QS::kMonsterPriority);
					break;
				}
			}
			case 1: {
				AIhero = AIHero::createAIHero(Vec2(0, 0), "Hero2");
				if (AIhero == nullptr)
				{
					log("monster picture not found");
				}
...
bool MapLayer::onContactBegin(PhysicsContact& contact)
{
	...
	if (nodeA != nullptr && nodeB != nullptr)
	{
			...
			|| ((nodeB)->getTag() == QS::kHeroTag && (nodeA)->getTag() == QS::kMonsterBulletTag))
		{
			...
			if (isEffect)
			{
				AudioEngine::play2d(QS::sound::kHurt);
			}
			if (nodeA->getTag() == QS::kHeroTag)
			{
			...
			|| ((nodeB)->getTag() == QS::kBulletBoxTag && (nodeA)->getTag() == QS::kHeroTag))
		{
			...
			if (isEffect)
			{
				AudioEngine::play2d(QS::sound::kOpenBox);
			}
