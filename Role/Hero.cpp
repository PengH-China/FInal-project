...
void Hero::move(int face, const char* pAnimateName,
	...
	animation->setRestoreOriginalFrame(true);//back to the original image
	...
	auto finalAction = Spawn::create(moveBy, pMoveAction, nullptr);
...
void Hero::roleMoveUpdate(float delta) {//detect every seconds what have done
	...
	else if (isKeyPressed(leftA)) {
		log("Left!!");
		keyPressedDuration(leftA);
		return;
	}
	...
	else if (isKeyPressed(rightD)) {
		keyPressedDuration(rightD);
		log("Right!!");
		return;
	}
	...
	else if (isKeyPressed(upW)) {
		keyPressedDuration(upW);
		log("Up!!");
		return;
	}
	...
	else if (isKeyPressed(downS)) {
		keyPressedDuration(downS);
		log("Down!!");
		return;
	}
	else {
